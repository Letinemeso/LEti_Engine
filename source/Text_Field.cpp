...
{
	set_texture(LEti::Resource_Loader::get_data<std::string>(_object_name, "font").first->c_str());
...
void Text_Field::set_text(const char* _text)
{
	...
	set_texture_coords(temp_coords, text.size() * tcpc);
	delete[] temp_coords;
	...
	for (unsigned int i = 2; i < text.size() * cpc; i += 3)
		temp_coords[i] = 0.0f;
	init_vertices(temp_coords, text.size() * cpc);
	delete[] temp_coords;
}
