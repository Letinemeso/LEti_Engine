...
{
	float vec_length = Math::vector_length(_vec);
...
{
	ASSERT(Math::vector_length(_first) == 0 ||  Math::vector_length(_second) == 0);

    //  (_first * _second) / (|_first| * |_second|)
    float divident = _first.x * _second.x + _first.y * _second.y + _first.z * _second.z;
	float divider = Math::vector_length(_first) * Math::vector_length(_second);
